...
struct node* head = NULL;   // head = TOP of the stack
struct node* f = NULL;
int dlinna = 0;

//ROTOTYPE
void spstore(void);          // push  — add to top
void review(void);           // print all
void del(char* name);        // delete by content
struct node* find(char* name);
struct node* get_struct(void);
void pop(void);              // pop   — remove from top

//HELPERS
void menu(void)
{
    printf("\n1-push 2-review 3-find 4-delete 5-pop 0-exit\nYour choice: ");
}
...
void spstore(void)
{
    ...
    p->next = head;    // new element points to old top
    head = p;       // new element becomes new top
    dlinna++;
